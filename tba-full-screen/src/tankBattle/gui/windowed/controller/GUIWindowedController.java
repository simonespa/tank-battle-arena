...
public final class GUIWindowedController extends ControllerAdapter {
	 ...
	 * questa classe sarï¿½ l'unico oggetto controller per questa GUI, e gestirï¿½
	 * tutti gli eventi che si possono verificare. Per fare ciï¿½, questa classe
	 * eredita da una classe Adapter, la quale implementa a corpo vuoto tutte le
	 * interfaccie disponibili per la gestione degli eventi. In questo modo
	 * possiamo utilizzare un solo ascoltatore, che gestisce ogni tipo di evento
	 * generato.
	 * 
	 * @param window
	...
	/**
	 * Gestisce gli eventi generati dalla perdita del focus
	 * delle combo box.
	 */
	@Override
	 ...
	 * 
	 * &Egrave; possibile inviare un messaggio di chat
	 * a tutti i componenti del gioco, grazie all'area di testo.	
	 */
	public void chat() {
	...
	private void startServer(ActionEvent event) {
		...
		/*
		 * altrimenti se è stato generato dal bottone nella finestra "Start Server"
		 */
		} else if (event.getSource() instanceof JButton) {
			// se ogni menù ha un valore valido
			if (TankBattleGame.typeIsNotNull() && TankBattleGame.modeIsNotNull() && TankBattleGame.mapIsNotNull()) {
				...
				if( TankBattleGame.getType().equalsIgnoreCase("team")) {
					 ...
					 * "StartServerWindow" solo per posizionare la finestra centrata
					 * rispetto ad essa.
					 */
	...
	private void nextToJoin() {
		...
		TankBattleGame.setJoinServerPanel((JoinServerPanel) joinServerWindow.getContentPane());
		mainWindow.setVisible(false);
	...
	private void leaveGame() {
		...
		int option = JOptionPane.showConfirmDialog(optionWindow,
				"Are you really sure to escape from the game?", mainWindow.getTitle()
	...
	/**
	 * Passa dalla modalitï¿½ "Windowed" a la modalitï¿½ "Full Screen"
	 */
