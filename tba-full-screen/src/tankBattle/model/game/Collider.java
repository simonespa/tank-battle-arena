...
public final class Collider implements ClientServerCapable {
	 ...
	 * 
	 * @param id
	 *            ï¿½ l'ID univoco del tank.
	 * @param r1
	 *            ï¿½ l'area di collisione del tank.
	 * @return {@code true} se il tank puï¿½ muoversi.
	 * @see {@link tankBattle.model.CollisionArea CollisionArea}
	 */
	...
	/*
	 * Controlla che il movimento dell'oggetto avvenga
	 * all'interno del riquadro di gioco.
	 */
	...
	public static synchronized void addTank(Tank tank) {
		...
		while (done) {
			if( isInside(tank.getArea()) ) {
				...
				for (Tank t : tanks) {
					// se c'è una sovrapposizione allora viene posto a true il flag
				...
				if (!intersection) {
					// controlla che il tank non collida con nessun ostacolo della
					// mappa
					for (GameComponent c : obstacles) {
						// se c'ï¿½ una sovrapposizione allora viene posto a true il
						// flag
						// intersection, che permette all'algoritmo di reiterare
						// nuovamente
	...
	/**
	 * Agiunge un ostacolo alla lista.
	 * 
	 * @param obstacle
	 *            l'ostacolo che viene aggiunto nella mappa.
	 */
	public static synchronized void addObstacle(GameComponent obstacle) {
		Random r = new Random(System.nanoTime());
		boolean done = true;
		boolean intersection = false;
		// itera fino a trovare la posizione corretta
		while (done) {
			// se l'ostacolo si trova all'interno dello schermo
			if( isInside(obstacle.getArea()) ) {
				// controlla che l'ostacolo non collida con nessun altro ostacolo
				for (GameComponent c : obstacles) {
					// se c'è una sovrapposizione allora viene posto a true il
					// flag intersection, che permette all'algoritmo di reiterare nuovamente
					if (obstacle.getArea().intersects(c.getArea())) {
						intersection = true;
						break;
					}
				}
			...
			if (!intersection) {
				...
				// altrimenti trova un'altra posizione all'ostacolo
				obstacle.setPosition(r.nextInt(maxX), r.nextInt(maxY));
			}
			intersection = false;
		}
		// infine aggiunge l'elemento
