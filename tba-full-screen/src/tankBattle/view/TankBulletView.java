...
public final class TankBulletView extends AbstractTankView {
	...
	/**
	 * Crea una vista alla quale viene delegato il compito di
	 * visualizzare i proiettili sparati dal tank.
	 * 
	 * @param width la larghezza del pannello di gioco.
	 * @param height l'altezza del pannello di gioco.
	...
	public synchronized void shoot() {
		if( bulletList.size() < 4 ) {
			Bullet b = new LeadBullet(tank.getID(), turret.getCenterX(), turret.getCenterY(), turret.getAngle(), this);
	...
	/**
	 * Elimina il proiettile sparato dalla lista
	 * dei proiettili attivi.
	 */
	public synchronized void killBullet() {
		Bullet b = bulletList.removeFirst();
		explosion = true;
		bulletExplosion = new BulletExplosion((Graphics2D) getGraphics(), b, this);
