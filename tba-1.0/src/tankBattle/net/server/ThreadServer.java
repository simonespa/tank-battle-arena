...
public final class ThreadServer extends BufferedIODemon implements
	...
	private void handshake() {
		...
		// invia al client l'ID, la mappa e l'ID della mappa
		String map = TankBattleGame.getMap();
		String mapdID = TankBattleGame.getMapID();
		String type = TankBattleGame.getType();
		String message = id + ":" + map + ":" + mapdID + ":" + type;
		...
		String regex = WORD + ":" + INTEGER + ":" + DECIMAL + ":" + DECIMAL
				+ ":" + INTEGER;
		...
		if (Pattern.matches(regex, message)) {
			...
			// invia il comando di creazione di un nuovo tank agli altri client
			// connessi
	...
	/*
	 * Estrapola i parametri dal messaggio e li assegna alle variabili locali.
	 */
