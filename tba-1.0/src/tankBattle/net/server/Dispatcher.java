			...
			if (message != null) {
				...
				synchronized (clientList) {
					...
					for (ThreadServer t : clientList) {
						// se il client non è lo stesso che ha inviato il
						// messaggio
						// allora
						if (!t.getTankID().equals(id)) {
							...
							if (message.startsWith(CREATE)) {
								// questo client riceve i dati di tutti gli
								// altri
								// client connessi
								// (tranne se stesso), in modo da ricreare in
								// locale
								// la lista dei nemici
