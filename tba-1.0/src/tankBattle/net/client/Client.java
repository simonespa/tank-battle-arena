...
import tankBattle.gui.windowed.ClientChooseTeamWindow;
...
public final class Client extends BufferedIODemon implements
	...
	private void handshake() {
		...
		if (message != null) {
			...
			if (Pattern.matches(INTEGER + ":" + WORD + ":" + WORD + ":" + WORD,
					message)) {
				createTankAndMap(message);
				sendTankParameter();
				TankBattleGame.startGameWindow();
				/*
				 * String type = message.split(":")[3]; startGameWindow(type);
				 */
			} else {
	...
	private void createTankAndMap(String message) {
		...
		String id = split[0];
		String map = split[1];
		String mapID = split[2];

		TankBattleGame.setMap(map);
		TankBattleGame.setMapID(mapID);
	...
	/*
	 * Fa partire la finestra di gioco.
	 */
	private void startGameWindow(String type) {
		if (type.equals("Team")
				&& TankBattleGame.getGameMode().equals("client")) {
			ClientChooseTeamWindow.showWindow();
			TankBattleGame.startGameWindow();
		} else {
			TankBattleGame.startGameWindow();
		}
